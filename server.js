...
alexaApp.intent('claimIdIntent', function (request, response) {
    ...
    var say = ["<s> According to our records, the current status of claim with ID <say-as interpret-as='digits'> "+ claimId +" is “ON HOLD”.  The reason for the same is <break strength=\"medium\" /> “Invoice Not Submitted”. Once the invoice is submitted, it will take 5 working days for settlement.</s>"];
    response.say(say.join('\n'));
});
